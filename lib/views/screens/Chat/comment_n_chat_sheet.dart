...
class CommentsNChatSheet extends StatefulWidget {
  final String? postId;
  final bool? isAdmin;
  final String? chatId;
...
//  final String name;
  ...
  @override
  CommentsNChatSheetState createState() => CommentsNChatSheetState(
        postId: this.postId,
...
class CommentsNChatSheetState extends State<CommentsNChatSheet> {
  final String? postId;
  final bool? isAdmin;
  final String? postMediaUrl;
...
//  final String name;
  ...
  buildChat() {
    ...
    return StreamBuilder<QuerySnapshot>(
      stream: chatRoomRef
          .doc(isAdmin! ? widget.chatId : currentUser!.uid)
          .collection("chats")
          .orderBy("timestamp", descending: false)
          .snapshots(),
      builder: (context, snapshot) {
  ...
  void sendNotificationToAdmin(
    ...
    bool isNotPostOwner = isAdmin != currentUser!.uid;
    if (isNotPostOwner) {
...
class _CommentsNMessagesState extends State<CommentsNMessages> {
  ...
  buildCommentBubble() {
    ...
    _isLiked = widget.likesMap![currentUser!.uid] == true;
    return Padding(
      padding: const EdgeInsets.all(8.0),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
                  ...
                  children: [
                    Row(
                      children: [
                        Text("${widget.name} : ",
                            style: TextStyle(
                                fontSize: 14.0, color: disabledTextColor)),
                        Flexible(
                          child: Text(
                            "${widget.comment}",
                            style: TextStyle(
                                fontSize: 14.0, color: lightTextColor),
                          ),
                        ),
                      ],
                    ),
                            ...
                            onTap: () async {
                              if (_isLiked) {
                                ...
                                await commentsRef
                                    ...
                                    .update({
                                  "likes": commentLikes,
                                  "likesMap": {currentUser!.uid: false}
                                });
